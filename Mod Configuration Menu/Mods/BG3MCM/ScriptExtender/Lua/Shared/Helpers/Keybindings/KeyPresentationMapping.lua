...
KeyPresentationMapping.Mapping = {
    ...
    MOUSE_BUTTON_9  = "Mouse 9",
    MOUSE_BUTTON_10 = "Mouse 10",
}
...
function KeyPresentationMapping:GetKBViewKey(keybinding)
    ...
    if keybinding.Button then
        return self:GetMouseViewKey(keybinding)
    end

    if (not keybinding.Key and not keybinding.ModifierKeys) or
...
function KeyPresentationMapping:GetMouseViewKey(mouseBinding)
    if not mouseBinding or not mouseBinding.Button or mouseBinding.Button == 0 then
        return ClientGlobals.UNASSIGNED_KEYBOARD_MOUSE_STRING
    end

    local buttonName = "MOUSE_BUTTON_" .. tostring(mouseBinding.Button)
...
function KeyPresentationMapping:GetViewKeyForSetting(settingId, modUUID)
    local keybinding = MCMAPI:GetSettingValue(settingId, modUUID)
    if not keybinding then
        return ClientGlobals.UNASSIGNED_KEYBOARD_MOUSE_STRING
    end
